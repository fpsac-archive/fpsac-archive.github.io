...
% Please use this years dmtcs.cls as using an older will result in the
% wrong header
...
\author{Jens Gustedt\addressmark{1}\thanks{I am not supported.}\and Somebody Who\addressmark{2}\thanks{But he is!}\and Some Dummy\addressmark{2}}

% We recommend that you don't put "extended abstract" in the title
...
\keywords{some well classifying words, \textbf{mandatory!}}
...
\paragraph{Abstract.}
  ...
  \end{description} The basic rule is that this abstract must be ``understandable on its
  own'' and this both for its contents \emph{and} for its
  TeXnicalities. In case your paper is published by us, it is used to
  produce the online abstract that is visible on the webpage of the
  journal and used for indexing.

% Here goes the French abstract
\paragraph{R\'esum\'e.}


%(We don't want Spanish abstracts this year)
\end{abstract}

\begin{center}
  \textbf{Don't include the following tableofcontents command
    in your file.}
\end{center}
This is only used for the purpose of this document here.
PDF documents as produced with this style will have an internal table
of contents.  Usually this table is shown by PDF browsers in a
different panel than the text itself.

The real text of your paper should already start here, on the
frontpage.

%%% Please eliminate the following two lines from your file.
\clearpage
\tableofcontents
...
\item The name(s) of the author(s), provided by the \verb!\author!
  ...
  \begin{center}
    \emph{Don't put homebrewn \LaTeX-commands or special chararcters here.}
  \end{center}
...
\end{verbatim}
  ...
  avoidable specialities in here.
...
\item An abstract of you manuscript, provided by the \verb!abstract!
  ...
  An abstract \emph{must} be understandable by its own and should not
  contain formulas or reference citations. In particular for
  citations, write things out such as `\emph{In a faboulous paper from
    2078, Samuel Bluetooth conjectured\ldots}''.  Also, the same rule
  as for the author command applies here, don't use avoidable
  specialities in here.
...
Have a look into \cite{oetiker99:_not_so_short_introd_latex} and
the
...
\section{PDF Files}
\label{sec:pdf}
The style supports \texttt{PDF} output if used with \texttt{pdflatex}
...
\begin{figure}[htbp]
  \begin{center}
    %% Don't use epsfile!
    \includegraphics[width=0.25\textwidth]{dmtcs}
...
The (standard) package \texttt{graphicx} is used by dmtcs.cls to include graphical data; please do \emph{not} use
...
\verb!\includegraphics[width=0.25\textwidth]{dmtcs}!. Note that in
...
final & being the final document for jounal papers\\
